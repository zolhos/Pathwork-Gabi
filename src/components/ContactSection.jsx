import { content } from '../data/content';

export default function ContactSection() {
  const { contact } = content;

  return (
    <section className="contact-section" id="contact" data-intent="transactional">
      <div className="container contact-container">
        <div className="contact-info-block">
          <span className="badge">ATENDIMENTO</span>
          <h2>{contact.ctaTitle}</h2>
          <p>{contact.ctaSubtitle}</p>
          
          <div className="cta-wrapper">
            <a 
              href={contact.whatsappUrl} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="btn btn-primary btn-large btn-icon"
              id="whatsapp-cta-btn"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" />
              </svg>
              Entrar em contato via WhatsApp
            </a>
          </div>
        </div>

        <div className="regions-block">
          <h3>Regiões Servidas e Formatos</h3>
          <div className="regions-list">
            {contact.regions.map((reg, idx) => (
              <div className="region-card" key={idx}>
                <div className="region-header">
                  <strong>{reg.city}</strong>
                  <span className="mode-badge">{reg.mode}</span>
                </div>
                <p className="formats-text">{reg.formats}</p>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
